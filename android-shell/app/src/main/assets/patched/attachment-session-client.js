...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
        ...
        function composerReady(sessionId) {
        ...
        function redispatchDrop(files, sessionId) {
		...
		function currentCwd(explicitSessionId) {
			const { sessionsService } = activeSession;
			if (sessionsService === undefined) return undefined;
			try {
				const snapshot = sessionsService.list.getSnapshot();
				const id = resolveSessionId(explicitSessionId);
		...
		async function intake(files, explicitSessionId) {
			...
			if (seq !== intakeSeqBySession.get(requestSession)) return;
			if (images.length > 0) {
				...
				try { redispatchDrop(images, sessionId); }
		...
		// ---- client plugin body ---------------------------------------------------
		const inject = ["slots", "sessions", "deckInput"];
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				const onDropCapture = (event) => {
					...
					if (files.length === 0) return;
					// Images also use the addressed intake instead of global drop listeners.
					event.preventDefault();
					event.stopImmediatePropagation();
					window.dispatchEvent(new Event("dragend")); // 复位内建 DropOverlay
					void intake(files, eventSessionId(event));
				};
				const onPasteCapture = (event) => {
					...
					if (files.length === 0) return;
					// Bind native image paste to the originating editor too.
					event.preventDefault();
					event.stopImmediatePropagation();
					const text = event.clipboardData?.getData("text/plain") ?? "";
					void intake(files, eventSessionId(event)).then(() => {
