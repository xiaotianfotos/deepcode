...
*/
/** Android catalog presentation policy, pinned to DSH 0.1.2-rc.1.
...
async function buildModelCatalog(ctx, defaultSelection = ctx.agentDefaultModel.currentSelection()) {
	...
	const catalog = await Promise.all(providers.map(async (provider) => {
		try {
            if (!await androidModelProviderConfigured(ctx, provider.id, androidDeclarations, androidNamespaces)) {
