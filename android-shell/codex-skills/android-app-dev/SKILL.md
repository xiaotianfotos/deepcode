...
> 首次使用先运行 doctor，并核实 Debian、SDK 输入和编译工具；下文环境描述是必需前提，不是新安装状态。imagegen 为可选能力，仅在当前运行时实际提供时使用，缺失时使用已有图标。
...
如果当前环境安装了 `imagegen` skill。需要 AI 生成的栅格应用图标时，读取其 SKILL.md 并调用**当前 Codex 的内置图像生成工具**。优先方形、主体居中、边缘留安全空间、缩小时轮廓可辨；按用户视觉要求设计，不凭空添加品牌。已经有原生 SVG 图标体系时按原项目方式维护。
