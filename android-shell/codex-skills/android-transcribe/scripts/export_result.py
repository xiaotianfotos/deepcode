...
def export(source,directory):
 ...
 units=[]
 source_offset=report.get('sourceOffsetSeconds',0)
 for segment in report['samples']:
  offset=source_offset+segment['offsetSeconds']
 ...
 assert all(source_offset<=w['start']<=w['end']<=source_offset+report['durationSeconds']+.001 for w in units)
 assert all(a['end']<=b['start']+.001 for a,b in zip(units,units[1:]))
 data={'schemaVersion':1,'language':report.get('language','Chinese'),'unit':'Chinese characters / English words','timestampUnit':'seconds','timestampResolutionSeconds':.08,'sourceOffsetSeconds':source_offset,'durationSeconds':report['durationSeconds'],'text':'\n'.join(s['text'] for s in report['samples']),'units':units,'note':'Model estimates, not manual ground truth. Raw punctuation times are retained only in benchmark report.'}
